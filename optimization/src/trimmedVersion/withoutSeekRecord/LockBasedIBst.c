...
static inline void seek(struct tArgs* t,unsigned long key,struct node** parentPtr, struct node** nodePtr)
{
	struct node* prev;
	struct node* curr;
	struct node* lastRNode;
...
static inline bool findSmallest(struct tArgs* t, struct node* node, struct node* rChild, struct node** succParentPtr, struct node** succNodePtr)
{
  ...
  while(true)
	{
		...
		else //complex delete
		{
			...
			isSplCase = findSmallest(t,node,rChild,&succParent,&succNode);
