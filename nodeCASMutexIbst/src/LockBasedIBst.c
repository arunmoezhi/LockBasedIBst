...
static inline void seek(struct tArgs* t,unsigned long key)
{
	...
	while(true)
	{
		...
		if(done)
		{
			...
			t->mySeekRecord->injectionPoint = temp;
			return;
		}
		t->readRetries++;
	}
...
bool insert(struct tArgs* t, unsigned long key)
{
	...
	while(true)
	{
		...
		if(lock(node))
		{
			if(node->key == nKey && node->child[which] == t->mySeekRecord->injectionPoint )
			{
  ...
  while(true)
	{
		...
		if(lock(node))
		{
			...
			else //complex delete
			{
				...
				if(isNull(succNode->child[RIGHT]))
				{
					succParent->child[isSplCase] = setNull(succParent->child[isSplCase]);
				}
				else
				{
					succParent->child[isSplCase] = succNode->child[RIGHT];
				}
				assert(succParent->child[isSplCase] != succNode);
				unlock(succParent);
				if(!isSplCase)
				{
					unlock(node);
...
void printKeysInOrder(struct node* node)
{
  ...
  printf("%lu\t%x\t%d\t%x\t%x\n",node->key,(struct node*) node,(bool) node->lockVar,(struct node*) node->child[LEFT],(struct node*) node->child[RIGHT]);
